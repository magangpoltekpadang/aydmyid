...
@section('content')
    <div x-data="{ ...outletData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Total Outlets</p>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Active Outlet</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="outlets.filter(v => v.is_active).length">
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Inactive Outlet</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="outlets.filter(v => !v.is_active).length">
                    </p>
                </div>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-white-500 uppercase border-b dark:border-purple-600 bg-purple-50 dark:text-white dark:bg-purple-600">
                            <th class="px-4 py-3">Outlet Name</th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                        <template x-for="(outlet, index) in outlets" :key="outlet.outlet_id">
                            <tr class="text-black-500 dark:text-black-100">
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="outlet.outlet_name"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="outlet.address"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="outlet.phone_number"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="outlet.latitude"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="outlet.longitude"></td>
                                <td class="px-4 py-3 text-xs">
                                    <span
                                        x-bind:class="outlet.is_active ? 'bg-green-600 text-green-100' :
                                ...
                                <td class="px-4 py-3 text-sm text-gray-500">
                                    <button @click="startEdit(outlet)" class="text-blue-600 hover:text-blue-900 mr-3">
                        ...
                        <tr x-show="outlets.length === 0">
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">No Outlets Found
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Outlet.create') {{-- Memanggil form dari file create.blade.php --}}
            </div>
        </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Outlet.edit') {{-- Memanggil form dari file edit.blade.php --}}
            </div>
        </div>
    </div>
