...
@section('content')
    <div x-data="{ ...customerData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Total Customers</p>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Customer Members</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="customers.filter(v => v.is_member).length">
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Customer Non-member</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="customers.filter(v => !v.is_member).length">
                    </p>
                </div>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-white-500 uppercase border-b dark:border-purple-600 bg-purple-50 dark:text-white dark:bg-purple-600">
                            ...
                            <th class="px-4 py-3">Vehicle Type Id</th>
                            <th class="px-4 py-3">Vehicle Color</th>
                            <th class="px-4 py-3">Member Number</th>
                            <th class="px-4 py-3">Join Date</th>
                            <th class="px-4 py-3">Member Expiry Date</th>
                            <th class="px-4 py-3">Status</th>
                            <th class="px-4 py-3">Action</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                        <template x-for="(customer, index) in customers" :key="customer.customer_id">
                            <tr class="text-black-500 dark:text-black-100">
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.plate_number"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.name"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.phone_number"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.vehicle_type_id"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.vehicle_color"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.member_number"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.join_date"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="customer.member_expiry_date"></td>
                                <td class="px-4 py-3 text-xs">
                                    <span
                                        x-bind:class="customer.is_member ? 'bg-green-600 text-green-100' :
                                            'bg-red-600 text-red-100'"
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        x-text="customer.is_member ? 'Member' : 'N-mem'">
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500">
                                    <button @click="startEdit(customer)" class="text-blue-600 hover:text-blue-900 mr-3">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button @click="confirmDelete(customer.customer_id)"
                        ...
                        <tr x-show="customers.length === 0">
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">No Customers Found
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Customer.create') {{-- Memanggil form dari file create.blade.php --}}
            </div>
        </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Customer.edit') {{-- Memanggil form dari file edit.blade.php --}}
            </div>
        </div>
    </div>
