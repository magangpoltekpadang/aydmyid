...
@section('content')
    <div x-data="{ ...staffData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Total Staffs</p>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Active Staffs</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="staffs.filter(v => v.is_active).length">
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Inactive Staffs</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="staffs.filter(v => !v.is_active).length">
                    </p>
                </div>
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                        <template x-for="(staff, index) in staffs" :key="staff.staff_id">
                            <tr class="text-black-500 dark:text-black-100">
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.name"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.email"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.phone_number"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.outlet_id"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.role_id"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="staff.last_login"></td>
                                <td class="px-4 py-3 text-xs">
                                    <span
                                        x-bind:class="staff.is_active ? 'bg-green-600 text-green-100' :
                                            'bg-red-600 text-red-100'"
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        x-text="staff.is_active ? 'Active' : 'Inactive'">
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500">
                                    <button @click="startEdit(staff)" class="text-blue-600 hover:text-blue-900 mr-3">
                        ...
                        <tr x-show="staffs.length === 0">
                            <td colspan="9" class="px-6 py-4 text-center text-sm text-gray-500">No Staff Found
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Staff.create') {{-- Memanggil form dari file create.blade.php --}}
            </div>
        </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Staff.edit') {{-- Memanggil form dari file edit.blade.php --}}
            </div>
        </div>
    </div>
