...
@section('content')
    <div x-data="{ ...serviceData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
        <!-- Header and Create Button -->
        <meta name="csrf-token" content="{{ csrf_token() }}">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-800">Service</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Total Services</p>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Active Services</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="services.filter(v => v.is_active).length">
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-700">Inactive Services</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="services.filter(v => !v.is_active).length">
                    </p>
                </div>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-white-500 uppercase border-b dark:border-purple-600 bg-purple-50 dark:text-white dark:bg-purple-600">
                            ...
                            <th class="px-4 py-3">Service Type Id</th>
                            <th class="px-4 py-3">Price</th>
                            <th class="px-4 py-3">Amount</th>
                            <th class="px-4 py-3">Estimated Duration</th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                        <template x-for="(service, index) in services" :key="service.service_id">
                            <tr class="text-black-500 dark:text-black-100">
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.service_name"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.service_type_id"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.amount"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.price"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.estimated_duration"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.description"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="service.outlet_id"></td>
                                <td class="px-4 py-3 text-xs">
                                    <span
                                        x-bind:class="service.is_active ? 'bg-green-600 text-green-100' :
                                            'bg-red-600 text-red-100'"
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        x-text="service.is_active ? 'Active' : 'Inactive'">
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500">
                                    <button @click="startEdit(service)" class="text-blue-600 hover:text-blue-900 mr-3">
                        ...
                        <tr x-show="services.length === 0">
                            <td colspan="9" class="px-6 py-4 text-center text-sm text-gray-500">No Service Found
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Service.create') {{-- Memanggil form dari file create.blade.php --}}
            </div>
        </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('Service.edit') {{-- Memanggil form dari file edit.blade.php --}}
            </div>
        </div>
    </div>
