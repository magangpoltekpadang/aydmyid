...
@section('content')
    <div x-data="{ ...serviceTypeData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
        <!-- Header and Create Button -->
        <meta name="csrf-token" content="{{ csrf_token() }}">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-800">Service Type</h1>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-white-800">
                ...
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-600"
                        x-text="serviceTypes.filter(v => v.is_active).length">
                    </p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class ="flex items-end">
                    <button @click="fetchServiceTypes()"
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                        <template x-for="(serviceType, index) in serviceTypes" :key="serviceType.service_type_id">
                            <tr class="text-black-500 dark:text-black-100">
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="serviceType.type_name"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="serviceType.code"></td>
                                <td class="px-4 py-3 text-sm text-gray-500" x-text="serviceType.description || '-'"></td>
                                <td class="px-4 py-3 text-xs">
                                    <span
                                        x-bind:class="serviceType.is_active ? 'bg-green-600 text-green-100' :
                                            'bg-red-600 text-red-100'"
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        x-text="serviceType.is_active ? 'Active' : 'Inactive'">
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500">
                                    <button @click="startEdit(serviceType)" class="text-blue-600 hover:text-blue-900 mr-3">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button @click="confirmDelete(serviceType.service_type_id)"
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('ServiceType.create') {{-- Memanggil form dari file create.blade.php --}}
            </div>
        </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
                @include('ServiceType.edit') {{-- Memanggil form dari file edit.blade.php --}}
            </div>
        </div>
    </div>

    @push('scripts')
        <script src="{{ asset('js/servicetype/service-type-script.js') }}"></script>
    @endpush
@endsection
