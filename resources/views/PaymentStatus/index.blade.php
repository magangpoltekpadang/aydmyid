...
@section('content')
<div x-data="{ ...paymentStatusData(), showCreateModal: false, showEditModal: false }" class="space-y-6">
    <!-- Header and Create Button -->
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Payment Status</h1>
        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-white-700 dark:bg-white-800">
                    <template x-for="(paymentStatus, index) in paymentStatuses" :key="paymentStatus.payment_status_id">
                        <tr class="text-black-500 dark:text-black-100">
                            <td class="px-4 py-3 text-sm text-gray-500" x-text="paymentStatus.status_name"></td>
                            <td class="px-4 py-3 text-sm text-gray-500" x-text="paymentStatus.code"></td>
                            <td class="px-4 py-3 text-sm text-gray-500" x-text="paymentStatus.description || '-'"></td>
                            <td class="px-4 py-3 text-xs">
                                <button @click="startEdit(paymentStatus)" class="text-blue-600 hover:text-blue-900 mr-3">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                <button @click="confirmDelete(paymentStatus.payment_status_id)" class="text-red-600 hover:text-red-900">
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button @click="deletePaymentStatus()" 
    ...
    <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
        <div @click.away="showCreateModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
            @include('PaymentStatus.create') {{-- Memanggil form dari file create.blade.php --}}
        </div>
    </div>
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
        <div @click.away="showEditModal = false" class="bg-white rounded-lg shadow w-full max-w-md mx-4">
            @include('PaymentStatus.edit') {{-- Memanggil form dari file edit.blade.php --}}
        </div>
    </div>
</div>

@push('scripts')
<script src="{{ asset('js/paymentstatus/payment-status-script.js') }}">
</script>
@endpush
@endsection